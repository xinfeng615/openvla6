...
def get_vla_normalized_action(vla, processor, base_vla_name, obs, task_label, action_dim=7):
    """Generate an action without applying any dataset-specific un-normalization.

    This is useful for strict zero-shot evaluation on a new embodiment whose
    action statistics are not present in the base OpenVLA checkpoint.
    """
    ...
    # Match the special empty token inserted by OpenVLA's predict_action().
    ...
    return vla.bin_centers[discretized_actions].astype(np.float32)
